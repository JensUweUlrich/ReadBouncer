...
// for Qt
void classify_reads_qt(std::string ibf_deplete_file_name, std::string ibf_target_file_name,  double  kmer_significance,
                       double error_rate, std::string classified_file_name, std::string unclassified_file_name, std::string read_file_name,
                        int preLen, int threads)
{
    std::shared_ptr<spdlog::logger> nanolive_logger = spdlog::get("NanoLiveLog");
    // initialize depletion and target filters
    interleave::IBFConfig DepleteIBFconfig{};
    ...
    // parse depletion IBF if given as parameter
    if (ibf_deplete_file_name.length() > 0)
    {
        try
        {
            DepleteIBFconfig.input_filter_file = ibf_deplete_file_name;
            interleave::FilterStats stats = DepleteFilter.load_filter(DepleteIBFconfig);
            DepletionFilters.emplace_back(DepleteFilter.getFilter());
            interleave::print_stats(stats);
            deplete = true;
    ...
    if (ibf_deplete_file_name.length() > 0)
    {
        try
        {
            TargetIBFconfig.input_filter_file = ibf_target_file_name;
            interleave::FilterStats stats = TargetFilter.load_filter(TargetIBFconfig);
            TargetFilters.emplace_back(TargetFilter.getFilter());
            interleave::print_stats(stats);
            target = true;
    ...
    interleave::ClassifyConfig Conf{};
    Conf.strata_filter = -1;
    Conf.significance = kmer_significance;
    ...
    // only print classified reads to file if option given via command line
    if (classified_file_name.length() > 0)
    {
        if (!seqan::open(ClassifiedOut, seqan::toCString(classified_file_name)))
        {
            std::cerr << "ERROR: Unable to open the file: " << classified_file_name << std::endl;
            return;
        }
    }
    // only print unclassified reads to file if option given via command line
    if (unclassified_file_name.length() > 0)
    {
        if (!seqan::open(UnclassifiedOut, seqan::toCString(unclassified_file_name)))
        {
            std::cerr << "ERROR: Unable to open the file: " << unclassified_file_name << std::endl;
            return;
        }
    }

    seqan::SeqFileIn seqFileIn;
    if (!seqan::open(seqFileIn, seqan::toCString(read_file_name)))
    {
        std::cerr << "ERROR: Unable to open the file: " << read_file_name << std::endl;
        return;
    }
    ...
    while (!seqan::atEnd(seqFileIn))
    {
        ...
        try
        {
            ...
            if (deplete && target)
            {
                ...
                else if (unclassified_file_name.length() > 0)
                {
                    std::stringstream sstr;
                    sstr << r.getID() << " channelNr=" << r.getChannelNr() << " readNr=" << r.getReadNr();
                    seqan::writeRecord(UnclassifiedOut, sstr.str(), r.getSeq());
    ...
    std::cout << "------------------------------- Final Results -------------------------------" << std::endl;
    std::cout << "Number of classified reads                         :   " << found << std::endl;
    std::cout << "Number of of too short reads (len < " << preLen << ")           :   " << too_short << std::endl;
    std::cout << "Number of all reads                                :   " << readCounter << std::endl;
    std::cout << "Average Processing Time Read Classification        :   " << avgClassifyduration << std::endl;
    std::cout << "-----------------------------------------------------------------------------------" << std::endl;
}
