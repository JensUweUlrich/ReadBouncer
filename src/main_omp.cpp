 ...
 //mtx.lock();
	...
	if(std::string("bloom_minhash").compare(args.mode) == 0)
	{
       ...
       // debug_stream<<"The used shape is : " <<s<<"\n";
		...
		//mtx.lock();
        output_results <<std::views::reverse(s)<< " " <<"|" << sum_num_containments << " " <<"|" << sketch_size<<" " <<"|" <<NbMinimizer<< "\n";
		//mtx.unlock();
...
int main(int argc, char const ** argv)
{
	...
	else
	{
       ...
       omp_set_num_threads(args.threads_th);
	   #pragma omp parallel
	   #pragma omp for
	   for(auto s = shapes_vector.begin(); s < shapes_vector.end(); ++s){
	  ...
	  /*  omp_set_num_threads(args.threads_th);
			//omp_set_num_threads(args.threads_th); // wrong in this case all CPUs will be used!
			//#pragma omp for
			//#pragma omp loop
			for (seqan3::shape s : shapes_vector)
			{
			    ...
			    sum_num_containments = 0;
			    sketch_size =0;
				//#pragma omp parallel private (sum_num_containments, sketch_size)
	   ...
	   /*  ThreadPool pool {args.threads_th};
		...
		{ 
		 ...
		 pool.enqueue([&args, s]()
		 {
			 ...
			 //mtx.lock();
			 //mtx.lock();
			 //std::thread th1(&run_program, &s, &args);
			 run_program(args,s);
