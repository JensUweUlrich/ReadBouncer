...
std::vector<dna4_vector> Minimizer::getMinimizer(dna4_vector & text, const bool ungapped)
{
	...
	dna4_vector revComp = text | std::views::reverse | views::complement;
	...
	for (uint32_t i = 0; i < windowKmers; ++i)
	{
		...
		// get current kmer
		dna4_vector curKmer = text | views::slice(i, i + k) | seqan3::views::convert<seqan3::dna4>;
		dna4_vector curRevComKmer = revComp | views::slice(revComp.size() - i - k, revComp.size() - i) | seqan3::views::convert<seqan3::dna4>;
	...
	for (uint64_t i = 1; i < possible; ++i)
	{
		...
		dna4_vector curKmer = text | views::slice(windowKmers + i - 1, windowKmers + i + k - 1) | seqan3::views::convert<seqan3::dna4>;
